...
def apply_file_changes(files: List[dict]) -> List[str]:
    modified = []
    for item in files:
...
"""
        ...
        try:
            ...
            modified_files = apply_file_changes(files)
    ...
    try:
        ...
        completion_comment = f"""## 🎉 [Gemini 3.7 Flash] 작업 완료 및 배포 성공 안내

요청하신 이슈 (#{issue_number})의 작업 및 배포가 성공적으로 완료되었습니다!
